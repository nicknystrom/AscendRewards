...
(function($) {
    $.jQTouch = function(options) {
        ...
        function addPageToHistory(page, animation, reverse) {
            ...
            // only add pages to history if they differ
            if (hist.length == 0 ||
                hist[0].hash != newHistoryItem.hash) {
               hist.unshift(newHistoryItem);           
            }
        }
        ...
        function doNavigation(fromPage, toPage, animation, backwards) {
            ...
            var fromAttached = fromPage.isAttached();
            if (!fromAttached) {
               _debug('fromPage is not part of the DOM.');
            }
            ...
            if ($.support.animationEvents && animation && jQTSettings.useAnimations) {
                ...
                // Bind internal "cleanup" callback, bind to toPage if the
                // fromPage is already removed from the dom (occurs when
                // an ajax request overwrites a page with the same id)
                (fromAttached ? fromPage : toPage).bind('webkitAnimationEnd', navigationEndHandler);
        ...
        function insertPages(nodes, animation) {
            ...
            $(nodes).each(function(index, node) {
                ...
                var $existing = $('#' + $node.attr('id')).remove();
        ...
        $(document).ready(function() {
            ...
            $.fn.isAttached = function() {
               return $.contains(document.body, this[0]);
            }
